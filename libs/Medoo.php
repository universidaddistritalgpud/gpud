...
class Medoo
{
	...
	public function insert($table, $datas, $return='')
	{
		$lastId = array();
		...
		foreach ($datas as $data)
		{
			...
			$response = $this->exec('INSERT INTO ' . $this->table_quote($table) . ' (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')');

			if($return == "stateQuery"){
				return $response;
			}
			
			$lastId[] = $this->pdo->lastInsertId('t2ir_user_id_seq');
		}
