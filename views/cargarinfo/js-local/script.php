...
var request_uri = location.protocol + "//" + location.hostname + ":" + location.port + location.pathname;
	...
	$.ajax({
        ...
        success: function(data)   
        {
        	...
        	if(data.notLoadOceanic){
        		$('#estacion_false').modal('show');
        	}else if(data.notEphemeris){
	           	$('#efemerides_false').modal('show');
        	}else{
        		$("#encrypt(botonprocesar)").hide();
